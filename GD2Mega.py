...
class Mega:
	...
	def __init__(self, username=None, password=None, local_folder="",
					remote_folder="", db_name='accounts.db'):
	...
	def account_details(self, email=None, password=None, print_res=True):
		...
		mega_path = 'C:\\Users\\nitish.mahto\\Downloads\\megatools\\megatools.exe'
		cmd = f"C:\\Users\\nitish.mahto\\Downloads\\megatools\\megatools.exe megadf -u {email} -p {password}"
		...
		used_space = (col.Fore.RED + self.readable_size(used) + col.Fore.RESET) \
						if used >= 21474836480 else self.readable_size(used)
		output = (
...
def upload(email, password, folder, mega_folder, register=False, db_name='accounts.db'):
	mega = Mega(email, password, folder, mega_folder, db_name)
...
if __name__ == '__main__':
	folds = []
	# accounts = []
	accounts = {f'mega.nitish.{i:0>2}@hi2.in':'password' for i in range(1,4)}

	Mega().check_all_accounts_details(accounts)

	while 0:
		fold = input("Enter folder path : ")
		user = input("Enter username : ")
		password = input("Enter Password : ")
		db = input("Database [accounts.db] : ")

		if fold == '' or user == '' or password == '':
			break

		mega_path = input("Mega path to put folder : ")
		register = True if input("Wanna register first[y|n] : ") == 'y' else False;

		upload(email, password, fold, mega_path, register=register, db_name=db)
